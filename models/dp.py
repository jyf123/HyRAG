...
def calculate_loss(graph_logits, text_logits, criterion):
    ...
    total_train_graph_loss = criterion(graph_logits, gt)  #ground_truth
...
class GCLIP_withpertub(torch.nn.Module):
    def __init__(self, model) :
        super(GCLIP_withpertub, self).__init__()
        self.model = model
        
    def forward(self,batch):
        return self.model.encode_graph_withperturb(batch)

class GCLIP_whole(torch.nn.Module):
    def __init__(self, model) :
        super(GCLIP_whole, self).__init__()
        self.model = model
        
    def forward(self,batch):
        return self.model.encode_whole_graph(batch)