...
def drop_nodes(data):
    ...
    # remap edge indices according to idx_dict (old -> new consecutive indices)
    edge_np = edge_index.numpy()
    mapped = [[idx_dict[int(edge_np[0, n])], idx_dict[int(edge_np[1, n])]]
              for n in range(edge_np.shape[1])]
    if len(mapped) > 0:
        edge_index = torch.tensor(mapped, dtype=torch.long).t()
    else:
