...
def load_data(dataset, seed=0,with_abs_con=False):
    if dataset == 'cora':
        ...
        from data.data_utils.load_children import get_raw_text_children as get_raw_text
        class_desc = pd.read_csv("./processed_data/categories/children_categories.csv")
        classes = class_desc['name'].tolist()
        c_descs = ['' for _ in range(len(classes))]
    ...
    if with_abs_con:
        data, text = get_raw_text(use_text=True, seed=seed,with_abs_con=with_abs_con) 
    else:
