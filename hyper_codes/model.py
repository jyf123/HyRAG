...
from hyper_codes.utils.math_utils import arsinh, arcosh
...
class KGEModel(nn.Module):
    def __init__(self, model_name, hidden_dim, gamma, nentity=None, nrelation=None,
                 double_entity_embedding=False, double_relation_embedding=False,
                 rid2cid=None, args=None,node_embedding=None,relation_embedding=None):
        ...
        self.register_buffer('entity_embedding', node_embedding)
        # self.entity_embedding = node_embedding
        # nn.init.uniform_(
        ...
        self.register_buffer('relation_embedding', relation_embedding)
        ...
        # 定义 MLP
        # 结构建议：Linear -> ReLU -> Dropout(可选) -> Linear
        # 最终输出维度必须严格等于 self.entity_dim 和 self.relation_dim
        input_ent_dim = self.entity_embedding.size(1) if self.entity_embedding is not None else 384
        input_rel_dim = self.relation_embedding.size(1) if self.relation_embedding is not None else 384
        self.entity_mlp = nn.Sequential(
            nn.Linear(input_ent_dim, hidden_dim),
            nn.LayerNorm(hidden_dim),  # <--- 新增：强行放大信号
            nn.ReLU(),
            nn.Linear(hidden_dim, self.entity_dim),
            # nn.Tanh()
        )
        
        self.relation_mlp = nn.Sequential(
            nn.Linear(input_rel_dim, hidden_dim),
            nn.LayerNorm(hidden_dim),  # <--- 新增：强行放大信号
            nn.ReLU(),
        ...
        # # 初始化 MLP 的参数  实际经验说明，这个确实有用
        # for m in self.entity_mlp.modules():
        #     if isinstance(m, nn.Linear):
        #         nn.init.xavier_uniform_(m.weight)
        #         nn.init.zeros_(m.bias)
        # for m in self.relation_mlp.modules():
        #     if isinstance(m, nn.Linear):
        #         nn.init.xavier_uniform_(m.weight)
        #         nn.init.zeros_(m.bias)
        if isinstance(self.entity_mlp[-1], nn.Linear):
            nn.init.uniform_(self.entity_mlp[-1].weight, -1e-4, 1e-4)
            nn.init.zeros_(self.entity_mlp[-1].bias)

        # 修正 Relation MLP 最后一层 (这是解决 1e-6 梯度的核心)
        if isinstance(self.relation_mlp[-1], nn.Linear):
            nn.init.uniform_(self.relation_mlp[-1].weight, -1e-4, 1e-4)
            nn.init.zeros_(self.relation_mlp[-1].bias)

        # self.scale = nn.Parameter(torch.tensor([3.0]),requires_grad=True)  # log(10) = 2.3
    ...
    def forward(self, sample, mode='single', degree=None):
        ...
        if mode == 'single':
            ...
            #     index=sample[:,2]
            # ).unsqueeze(1)
            head = self.get_entity_embedding(sample[:,0]).unsqueeze(1)
            relation = self.get_relation_embedding(sample[:,1],self.model_name).unsqueeze(1)
            tail = self.get_entity_embedding(sample[:,2]).unsqueeze(1)
            ...
            head = self.get_entity_embedding(head_part.view(-1)).view(batch_size, negative_sample_size, -1)
            relation = self.get_relation_embedding(tail_part[:, 1],self.model_name).unsqueeze(1)
            tail = self.get_entity_embedding(tail_part[:, 2]).unsqueeze(1)
            ...
            head = self.get_entity_embedding(head_part[:,0]).unsqueeze(1)
            relation = self.get_relation_embedding(head_part[:,1],self.model_name).unsqueeze(1)
            tail = self.get_entity_embedding(tail_part.view(-1)).view(batch_size, negative_sample_size, -1)
    ...
    def RotC(self, head, relation, tail, mode): # a number of $dim$ 2D hyperbolic planes
        if isinstance(relation, tuple):
            ...
            relation = relation[0]  #[1024,1,1501]
        ...
        if self.args.learnable_curvature:
            ...
            c = self.curvature
        head, bh = head.split([int(2*self.entity_dim/3), int(self.entity_dim/3)], dim=2)#[1024,1,1500]->[1024,1,1000],[1024,1,500]
        tail, bt = tail.split([int(2*self.entity_dim/3), int(self.entity_dim/3)], dim=2)#[1024,50,1500]->[1024,50,1000],[1024,50,500]
        
        batch_size = head.size()[0]
        dim = int(head.size()[2] / 2)
        head = head.view(batch_size, -1, 2, dim).transpose(2, 3) #[1024,1,1000]->[1024,1,500,2]
        tail = tail.view(batch_size, -1, 2, dim).transpose(2, 3) #[1024,50,1000]->[1024,50,500,2]
        
        head = hyperbolic.proj(hyperbolic.expmap0(head, c), c=c)  #expmap0():将head从欧式空间映射到双曲空间 proj()把向量的模长约束到1以内（单位球内）这里的proj函数有问题
        tail = hyperbolic.proj(hyperbolic.expmap0(tail, c), c=c)

        head_ = (head.transpose(2, 3)).contiguous().view(batch_size, -1, 2*dim) #[1024,1,500,2]->[1024,1,1000]
        ...
        phase_relation = 2 * pi * torch.sigmoid(phase_relation) - pi
    ...
    def ConE(self, head, relation, tail, mode):
        ...
        head = hyperbolic.proj(hyperbolic.expmap0(head, c), c)
        tail = hyperbolic.proj(hyperbolic.expmap0(tail, c), c)
    ...
    def get_scaled_embeddings(self, embeddings, target_min=-0.5, target_max=0.5):
        """
        完全可导的版本
        """
        # 计算当前范围（保持为张量）
        current_min = embeddings.min()  # 保持为张量
        current_max = embeddings.max()  # 保持为张量
        ...
        # 添加小常数避免除零，同时保持梯度
        epsilon = torch.tensor(1e-8, device=embeddings.device)
        
        # 可导的缩放
        normalized = (embeddings - current_min) / (current_range + epsilon)
        scaled = normalized * target_range + target_min
        
        return scaled
    
    def get_entity_embedding_dir(self,emb):
        entity_embedding = self.entity_mlp(emb)
    ...
    def get_entity_embedding(self,index):
        ...
        entity_embedding = self.entity_mlp(entity_embedding)
        # scaling_factor = torch.exp(self.scale)
        # entity_embedding = self.get_scaled_embeddings(entity_embedding,target_min=-self.embedding_range.item(), target_max=self.embedding_range.item())
        return entity_embedding
        # return entity_embedding*self.embedding_range.item()
    def get_relation_embedding(self,index,model):
        relation_embedding = torch.index_select(
                self.relation_embedding, 
                dim=0, 
                index=index
            )
        relation_embedding = self.relation_mlp(relation_embedding)
        # scaling_factor = torch.exp(self.scale)
        # print(relation_embedding.abs().mean())
        # return relation_embedding
        if model != 'ConE':
            # relation_embedding = self.get_scaled_embeddings(relation_embedding,target_min=-self.embedding_range.item(), target_max=self.embedding_range.item())
            return relation_embedding
        else:
            # nn.init.uniform_(tensor=self.relation_embedding[:, hidden_dim:2*hidden_dim], a = 0.5, b = 1)
            # nn.init.uniform_(tensor=self.relation_embedding[:, 2*hidden_dim:3*hidden_dim], a = -0.1, b = 0.1)     
            part1 = relation_embedding[:, :self.hidden_dim]
            part2 = 0.5 + 0.5 * torch.sigmoid(relation_embedding[:, self.hidden_dim:2*self.hidden_dim])
            part3 = relation_embedding[:, 2*self.hidden_dim:]
            # part1 = relation_embedding[:, :self.hidden_dim] * self.embedding_range.item()
            # part2 = relation_embedding[:, self.hidden_dim:2*self.hidden_dim] * 0.25 + 0.75
            # part3 = relation_embedding[:, 2*self.hidden_dim:] * 0.1
