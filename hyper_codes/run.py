...
def parse_args(args=None):
    ...
    parser.add_argument('--cskg_path', default="cskg.tsv", type=str)
...
def read_class(file_path, entity2id):
    ...
    with open(file_path) as fin:
        for line in fin:
            ...
            triples.append((int(h), int(r), int(t), int(l)))  #l是什么？
    return triples
...
def process_cskg(data_path):
    # 提取实体和关系并生成 id 映射与关系列表
    entity2id = dict()
    with open(os.path.join(data_path, 'entities.dict')) as fin:
        for line in fin:
            eid, entity = line.strip().split('\t')
            entity2id[entity] = int(eid)
    entities_sorted = list(entity2id.keys())
    ...
    logging.info("CSKG: %d entities, %d relations" % (len(entity2id), len(relation2id)))
    
    # 生成三元组列表
    train_triples = read_triple(os.path.join(data_path, 'triplets.tsv'), entity2id, relation2id)
    logging.info("CSKG: %d training triples" % (len(train_triples)))
    # valid_triples = read_triple(os.path.join(data_path, 'valid.tsv'), entity2id, relation2id)
    # logging.info("CSKG: %d validation triples" % (len(valid_triples)))
    # test_triples = read_triple(os.path.join(data_path, 'test.tsv'), entity2id, relation2id)
    # logging.info("CSKG: %d testing triples" % (len(test_triples)))
    ...
    logging.info("Loading cached node embeddings from %s" % (node_attr_path))
    node_attr = torch.load(node_attr_path)

    logging.info("Loading cached edge embeddings from %s" % (edge_attr_path))
    edge_attr = torch.load(edge_attr_path)
    
    # 返回以供调用者使用（如果需要）
    return entity2id, relation2id, train_triples,  node_attr, edge_attr
...
def main(args):
    ...
    entity2id,relation2id,train_triples,node_embedding,relation_embedding =process_cskg(args.data_path)
    print(relation2id)

    rid2cid = None #[raltionid:categoryid]
    ...
    #             class_test_triples = [triple + (rid2cid[triple[1]],) for triple in class_test_triples]  #加上relation category
    ...
    # if args.do_test_relation_category:
    #     valid_triples = [triple + (rid2cid[triple[1]],) for triple in valid_triples]#加上relation category
    #     test_triples = [triple + (rid2cid[triple[1]],) for triple in test_triples]#加上relation category
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            ...
            # if (step+1)%1000==0:
            #     print(f"缩放因子为{kge_model.scale.item()}")
